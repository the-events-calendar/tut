...
class Package extends Command {
	...
	private function run_pre_build_installs( $plugin ) {
		...
		$pool = new Pool();
		$pool->add( new Process( '. ' . $this->get_nvm_path() . ' && nvm use && yarn cache clean && yarn install' ), [ 'yarn' ] );

		if ( $has_common ) {
			$pool->add( new Process( 'cd common && . ' . $this->get_nvm_path() . ' && nvm use && yarn cache clean && yarn install' ), [ 'yarn common' ] );
		}
