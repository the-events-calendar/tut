...
class Package extends Command {
	...
	private function run_pre_build_installs( $plugin ) {
		...
		$pool = new Pool();
		$pool->add( new Process( 'npm ci' ), [ 'npm' ] );

		if ( $has_common ) {
			$pool->add( new Process( 'cd common && npm ci' ), [ 'npm common' ] );
		}
	...
	private function run_gulp_tasks( $plugin ) {
		...
		if ( $has_common ) {
			...
			$this->output->writeln( '<fg=cyan>* Compiling common PostCSS</>', OutputInterface::VERBOSITY_VERBOSE );
			$this->output->writeln( '<fg=cyan>* Compressing common JS</>', OutputInterface::VERBOSITY_VERBOSE );
			$pool->add( new Process( './node_modules/.bin/gulp glotpress' ), [ 'glotpress common' ] );
			$pool->add( new Process( './node_modules/.bin/gulp postcss' ), [ 'postcss common' ] );
			$pool->add( new Process( './node_modules/.bin/gulp compress-js' ), [ 'compress-js common' ] );
		}
