...
class Package extends Command {
	...
	protected function execute( InputInterface $input, OutputInterface $output ) {
		...
		foreach ( $this->selected_plugins as $plugin ) {
			...
			if ( file_exists( 'Gruntfile.js' ) ) {
				...
				$process = $this->run_process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp zip' );
	...
	private function run_pre_build_installs( $plugin ) {
		...
		$pool = new Pool();
		$pool->add( new Process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && npm ci' ), [ 'npm' ] );

		if ( $has_common ) {
			$pool->add( new Process( 'cd common && ' . $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && npm ci' ), [ 'npm common' ] );
		}
	...
	private function run_grunt_tasks( $plugin ) {
		...
		if ( $has_common ) {
			...
			$pool->add( new Process( 'cd common && ' . $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp glotpress' ), [ 'glotpress common' ] );
			$pool->add( new Process( 'cd common && ' . $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp postcss' ), [ 'postcss common' ] );
			$pool->add( new Process( 'cd common && ' . $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp compress-js' ), [ 'compress-js common' ] );
		}
		...
		if ( $has_common ) {
			...
			$this->run_process( 'cd common && ' . $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp compress-css' );
		}

		$this->output->writeln( '<fg=cyan>* Running npm run build</>', OutputInterface::VERBOSITY_VERBOSE );
		$this->run_process($this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && npm run build' );

		$this->output->writeln('<fg=cyan>* Grunt packaging</>', OutputInterface::VERBOSITY_VERBOSE);
		$process = $this->run_process($this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && grunt build', true, 900 );
	...
	private function run_gulp_tasks( $plugin ) {
		...
		$pool->add( new Process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp glotpress' ), [ 'glotpress' ] );
		$pool->add( new Process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp postcss' ), [ 'postcss' ] );
		$pool->add( new Process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp compress-js' ), [ 'compress-js' ] );
		...
		$process = $this->run_process($this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp compress-css');
		...
		if ( file_exists( 'webpack.config.js' ) ) {
			...
			$process = $this->run_process( $this->get_source_command() . ' ' . $this->get_nvm_path() . ' && nvm use --silent --no-install && ./node_modules/.bin/gulp webpack', true, 900 );
		}
